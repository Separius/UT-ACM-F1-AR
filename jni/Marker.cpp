...
void Marker::CompareCorners(vector<PointDouble > &_marker_corners_img, int *orientation, double *error, int pointX, int pointY, double diameter) {
	...
	for (int i=0; i<4; i++) {
		//TODO check Performance vs PointSquaredFloat
	...
	*orientation = ranaMinElementIndex(errors[0], errors[1], errors[2], errors[3]);
	*error = sqrt(errors[*orientation])*0.5;

	*error /= (diameter);//TODO game engine Trick
...
bool Marker::UpdateContentBasic(vector<PointDouble > &_marker_corners_img, IplImage *gray, Camera *cam, int frame_no /*= 0*/) {
	...
	for (std::size_t i=0; i<marker_margin_w_img.size(); i++) {
	...
	for (std::size_t i=0; i<marker_margin_b_img.size(); i++) {
